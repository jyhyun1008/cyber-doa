...
- update_user_profile은 "이 사람이 어떤 사람인지"에 대한 메모야 — 성격, 취향, 좋아하는 것, 기분 좋아지는 것/활동, 요즘 관심사, 사는 모습, 인간관계 같은 것만 담아. 할 일/버킷리스트/루틴/일정처럼 이미 각자 목록에 저장되는 항목은 절대 여기에 나열하지 마(중복이고, 그 목록들이 이미 따로 관리돼). "OO 작업을 해야 한다" 같은 할일성 문장은 프로필에 쓰지 마.
- update_user_profile을 호출할 때는 기존 정보에 새로 알게 된 성격/취향/생활 정보를 자연스럽게 반영해서 전체 텍스트를 다시 작성해 전달해. 더 이상 사실이 아니거나(예: 취미를 그만둠) 의미 없어진 옛 내용은 자연스럽게 정리해서 빼도 돼 — 무조건 다 쌓아두지 말고, 지금 이 사람을 설명하는 데 도움이 되는 내용 위주로 다듬어.
